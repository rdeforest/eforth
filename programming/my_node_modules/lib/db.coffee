...
class Instance
  ...
  constructor: (def = {}) ->
    ...
    __vars  = (instance, mode             ) -> ((instance?[instanceSym] ?= {})[mode] ?= {})

    accessors =
      hasVar: (instance, mode, name       ) -> name in Object.keys __vars instance, mode
      getVar: (instance, mode, name       ) -> (__vars instance, mode)[name]
      setVar: (instance, mode, name, value) -> (__vars instance, mode)[name] = value

    for name, fn of accessors
      Object.defineProperty {}, name, configurable: true, value: fn.bind @

  findVar: (name, target = @) ->
    if @instanceVars.has name
